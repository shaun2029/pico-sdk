...
// param bssid is a pointer to an array of uint8_t containing the 6 bytes of the preffered BSSID MAC address. 
...
int cyw43_arch_wifi_connect_until(const char *ssid, const uint8_t *bssid, const char *pw, uint32_t auth, absolute_time_t until) {
    int err = cyw43_arch_wifi_connect_async(ssid, bssid, pw, auth);
    if (err) return err;
...
int cyw43_arch_wifi_connect_blocking(const char *ssid, const uint8_t *bssid, const char *pw, uint32_t auth) {
    return cyw43_arch_wifi_connect_until(ssid, bssid, pw, auth, at_the_end_of_time);
}

int cyw43_arch_wifi_connect_timeout_ms(const char *ssid, const uint8_t *bssid, const char *pw, uint32_t auth, uint32_t timeout_ms) {
